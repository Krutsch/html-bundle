...
function createGlobalJS(err, files) {
    ...
    files.forEach((filename) => {
        ...
        dynamicImportStatements &&
            imports.push(...dynamicImportStatements.map((importLine) => {
    ...
    imports.forEach((importLine) => {
        ...
        if (HTMLGlobalDependency.has(pkg)) {
            ...
            importsDestructured.forEach((distinctVarString) => pkgObj.distinctVars.add(distinctVarString));
        }
        else {
    ...
    HTMLGlobalDependency.forEach(({ global, distinctVars }, pkg) => {
      ...
      export default ${global}`;
            ...
            content = `export ${global ? `${global}${distinctVarsArr.length ? "," : ""}` : ""} ${distinctVarsArr.length ? `{ ${distinctVarsArr.join(",")} }` : ""} from "${pkg}";`;
        }
